...
| `create_strategy` | Mutation | `name` (string), `description` (string, optionnel), `custom_fields` (string, optionnel — champs de confluence personnalisés : liste JSON `["RSI","TENDANCE"]` ou liste séparée par virgules, mis en majuscules par `SetStructure`) | **oui** | `CreateStrategy(arguments)` |
