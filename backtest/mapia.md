...
| Élément | Description |
|---|---|
| **En-tête** | Orbe IA néon statique (double anneau + icône robot vectorielle, **aucun effet de blur ni animation** — rendu logiciel), titre « DATAEDGE **AI** », pastille verte « Copilote connecté · vos données restent locales », bouton **🗑** (`BtnClearChat_Click` → efface toute la discussion **serveur + agent**), bouton **⤢** (`ExpandRequested` → agrandit/réduit le panneau) et bouton **✕** (`CloseRequested` → masque le panneau) |
